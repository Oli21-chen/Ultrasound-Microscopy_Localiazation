...
x=layers.Conv2D(C, 3, strides=1, padding='same',activation=None, name="B1_conv1")(inputs)#128,128,64
x=layers.BatchNormalization(name='B1_bn1')(x)
x= layers.LeakyReLU(alpha=0.3,name='B1_relu1')(x)
x= layers.Conv2D(C, 3, strides=1,padding='same', activation=None, name="B1_conv2")(x)#128,128,64
x= layers.BatchNormalization(name='B1_bn2')(x)
x= layers.LeakyReLU(alpha=0.3,name='B1_relu2')(x)
x= layers.MaxPool2D(pool_size=(2, 2),name='B1_maxpool')(x)#64,64,64
#concat1=x


 #Conv block 2
x=layers.Conv2D(C*2, 3, strides=1,padding='same', activation=None, name="B2_conv1")(x)#64,64,128
x=layers.BatchNormalization(name='B2_bn1')(x)
x= layers.LeakyReLU(alpha=0.3,name='B2_relu1')(x)
x= layers.Conv2D(C*2, 3, strides=1,padding='same', activation=None, name="B2_conv2")(x)#64,64,128
x= layers.BatchNormalization(name='B2_bn2')(x)
x= layers.LeakyReLU(alpha=0.3,name='B2_relu2')(x)
x= layers.MaxPool2D(pool_size=(2, 2),name='B2_maxpool')(x)#32,32,128
#concat2=x

#Conv block 3
x=layers.Conv2D(C*4, 3, strides=1,padding='same', activation=None, name="B3_conv1")(x)#32,32,256
x= layers.BatchNormalization(name='B3_bn1')(x)
x= layers.LeakyReLU(alpha=0.3,name='B3_relu1')(x)
x= layers.Conv2D(C*4, 3, strides=1,padding='same', activation=None, name="B3_conv2")(x)
x= layers.BatchNormalization(name='B3_bn2')(x)
x= layers.LeakyReLU(alpha=0.3,name='B3_relu2')(x)
x= layers.MaxPool2D(pool_size=(2, 2),name='B3_maxpool')(x)#16, 16, 256
concat3=x


 #latent block #no any leak
x= layers.Conv2D(C*8, 3, strides=1, padding='same',activation="relu", name="latent_1")(x)#16,16,512
x= layers.Conv2D(C*8, 3, strides=1, padding='same',activation="relu", name="latent_2")(x)#16,16,512
x= layers.Dropout(0.5)(x)
#x=tf.keras.layers.concatenate([x, concat3])

 #Deconv block 1
x=layers.Conv2DTranspose(C*8,5,strides=1,padding='same',activation="relu",name="DeConv_1")(x)#16,16,512
x= layers.Conv2DTranspose(C*4,5,strides=2,padding='same',activation="relu",name="DeConv_2")(x)#32,32,256
x= layers.UpSampling2D(size=(2, 2),interpolation='nearest')(x)#64,64,256
#x=tf.keras.layers.concatenate([x, concat1])

 #Deconv block 2
x= layers.Conv2DTranspose(C*2,5,strides=1,padding='same',activation="relu",name="DeConv_3")(x)#64,64,128
x= layers.Conv2DTranspose(C,5,strides=2,padding='same',activation="relu",name="DeConv_4")(x)#128,128,64
x= layers.UpSampling2D(size=(2, 2))(x)#256,256,64


 #Deconv block 3
x= layers.Conv2DTranspose(C,5,strides=1,padding='same',activation="relu",name="DeConv_5")(x)#256,256,64
x= layers.Conv2DTranspose(C,5,strides=2,padding='same',activation="relu",name="DeConv_6")(x)#512,512,64
x= layers.UpSampling2D(size=(2, 2))(x)#1024,1024,64
...
#%%
x_train=[]
for root, dirnames, filenames in os.walk(r"C:\Users\Olive\Desktop\Fit2DGaussian Function to Data\Single_BB\20db_inputs"):
...
y_train=[]
for root, dirnames, filenames in os.walk(r'C:\Users\Olive\Desktop\Fit2DGaussian Function to Data\Single_BB\20db_outputs'): 
    for filename in filenames:
        if re.search("\.(jpg|jpeg|JPEG|png|PNG|JPG)$", filename):
            filepath = os.path.join(root, filename)
            image = plt.imread(filepath)
...
checkpointer = ModelCheckpoint(
    filepath=r"C:\Users\Olive\Desktop\Fit2DGaussian Function to Data\Single_BB\Model_Unet_20db", 
...
# keras_model_path = r"C:\Users\Olive\Desktop\Fit2DGaussian Function to Data\Single_BB\Noise_Single_Model"
...
for i in range(I.shape[0]):#I.shape[0]
    ...
    cv.imwrite(os.path.join(path , 'U_%d.jpg' % i), res)
