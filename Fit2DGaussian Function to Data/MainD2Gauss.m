...
x0 = [1,0,1,0,1,0]; %Inital guess parameters
...
xin = x; 
noise = noise/100 * x(1);
[X,Y] = meshgrid(-MdataSize/2:MdataSize/2);%meshgrid(x,y)
xdata = zeros(size(X,1),size(Y,2),2);
xdata(:,:,1) = X;
xdata(:,:,2) = Y;
[Xhr,Yhr] = meshgrid(linspace(-MdataSize/2,MdataSize/2,300)); % generate high res grid for plot n=#generated
...
% x = lsqcurvefit(fun,x0,xdata,ydata) starts at x0 and finds coefficients x to best fit the nonlinear function fun(x,xdata) 
%to the data ydata (in the least-squares sense). ydata must be the same size as the vector (or matrix) F returned by fun.
%% --- Fit---------------------
...
alpha(1)  % adjust color transparent
hold off
%% -----Plot profiles----------------
hf2 = figure(2);
